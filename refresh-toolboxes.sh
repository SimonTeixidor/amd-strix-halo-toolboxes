...
TOOLBOXES["llama-vulkan-radv-slot-checkpoints"]="docker.io/kyuz0/amd-strix-halo-toolboxes:vulkan-radv-slot-checkpoints --device /dev/dri --group-add video --security-opt seccomp=unconfined"
...
function usage() {
  ...
  echo "  - llama-vulkan-radv-slot-checkpoints"
  exit 1
}
